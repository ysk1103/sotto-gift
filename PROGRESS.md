...
**提案カードのUX／多様性（調整済み）**：
- 価格を大きく表示（「商品を見る」横。セールは ~~通常~~→価格＋セール）。語りは薄い色の囲みで商品情報と区別。
- 楽天の長い商品名はノイズ除去＋整形（`acquire._clean_title`）＋表示2行省略。品名で重複排除。
- もらった履歴の学習は**ほんのり**：検索キーワードには使わず、カテゴリをFitに薄く混ぜるだけ（特定商品名で検索を狭めない）。あげた履歴は被り回避に使用。
- 多様性：MMR λ=0.6、類似は品名の似かた重視。似た商品の連続を防止。
- 開発中はサーバーが `Cache-Control: no-store`（CSS/JS変更が即反映）。
